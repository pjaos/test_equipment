...
class PersistentPSUCtrlConfig(object):
    ...
    def __init__(self, uio):
        ...
        self._uio = uio
        self._configManager = ConfigManager(self._uio, PersistentPSUCtrlConfig.CFG_FILE, PersistentPSUCtrlConfig.DEFAULT_CONFIG)
    ...
    def _enterPSUType(self):
        ...
        self._configManager.inputDecInt(PersistentPSUCtrlConfig.PSU_TYPE, "PSU TYPE", minValue=1, maxValue=PSUFactory.MAX_PSU_TYPE)
    ...
    def _enterPSUDeviceString(self):
        ...
        self._configManager.inputStr(PersistentPSUCtrlConfig.PSU_CONNECTION_STRING, "PSU connection string", False)
...
class PSU(object):
    ...
    def __init__(self, uio, options):
        ...
        self._psuType = self._pPSUConfig.getAttr(PersistentPSUCtrlConfig.PSU_TYPE)
        self._psuConnectionString = self._pPSUConfig.getAttr(PersistentPSUCtrlConfig.PSU_CONNECTION_STRING)
    ...
    def setOutput(self, on):
        ...
        if on:
            ...
            self._uio.info("Set PSU voltage to {:.2f} volts.".format(self._options.volts))
            self._psu.setAmps(self._options.amps)
            self._uio.info("Set PSU current limit to {:.2f} amps.".format(self._options.amps))
    ...
    def _getSerialDevice(self):
        ...
        if self._psuConnectionString.endswith(":INSTR"):
            ...
            serialDevice = SerialPortFinder.GetDevice(self._psuConnectionString)
...
def main():
    ...
    try:
        parser = argparse.ArgumentParser(description="A description of what it does.", formatter_class=argparse.RawDescriptionHelpFormatter)
        parser.add_argument("-d", "--debug",  help="Enable debugging.", action='store_true')
        parser.add_argument("-c", "--config", help="Configure the PSU parameters.", action='store_true')
        parser.add_argument("-v", "--volts",  help="Set PSU voltage (default=3.3).",        type=float, default=3.3)
        parser.add_argument("-a", "--amps",   help="Set PSU current limit (default=1.0).",  type=float, default=1.0)
        parser.add_argument("-o", "--output", help="The PSU output to use (default=1).",    type=int, default=1)
        parser.add_argument("--on",           help="Turn the output on.",                   action='store_true')
        parser.add_argument("--off",          help="Turn the output off.",                  action='store_true')
        parser.add_argument("-r", "--read",   help="Read the output current/power.",        action='store_true')
