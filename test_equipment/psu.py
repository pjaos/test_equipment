...
#            raise NotImplementedError('Subclass must define the PSU_NAME class attrbute.')
    ...
    def connect(self, dev):
        ...
        raise NotImplementedError("connect() method not implemented by {}".format(self.__class__.__name__))

    def disconnect(self):
        """@brief Disconnect from the power supply."""
        raise NotImplementedError("disconnect() method not implemented by {}".format(self.__class__.__name__))

    def selectOutput(self, output):
        """@brief Select PSU output.
           @param output The output to select. An integer value."""
        raise NotImplementedError("selectOutput() method not implemented by {}".format(self.__class__.__name__))

    def setVolts(self, voltage):
        """@brief Set the currently selected output voltage.
           @param voltage The voltage in volts."""
        raise NotImplementedError("setVolts() method not implemented by {}".format(self.__class__.__name__))

    def setAmps(self, current):
        """@brief Set the selected output current limit.
           @param current The current limit in amps."""
        raise NotImplementedError("setAmps() method not implemented by {}".format(self.__class__.__name__))
    ...
    def enableOutput(self, on):
        ...
        raise NotImplementedError("enableOutput() method not implemented by {}".format(self.__class__.__name__))

    def switchOn(self, on):
        """@brief Turn on/off all previously enabled outputs.
           @param on If True turn output/s on."""
        raise NotImplementedError("switchOn() method not implemented by {}".format(self.__class__.__name__))

    def getVolts(self):
        """@brief Get the selected output voltage.
           @return The selected output voltage in volts."""
        raise NotImplementedError("getVolts() method not implemented by {}".format(self.__class__.__name__))

    def getLimitAmps(self):
        """@brief Get the selected output current limit.
           @return The selected output current limit in amps."""
        raise NotImplementedError("getLimitAmps() method not implemented by {}".format(self.__class__.__name__))

    def getLoadAmps(self):
        """@brief Get the selected output load current.
           @return The selected output load current in amps."""
        raise NotImplementedError("getLoadAmps() method not implemented by {}".format(self.__class__.__name__))
...
class HMP2030PSU(PSUBase):
    ...
    def selectOutput(self, output):
        ...
        if output not in [1,2,3]:
            raise Exception(f"{output} is not a valid output. This PSU has outputs 1,2 or 3.")
    ...
    def setVolts(self, voltage):
        ...
        if voltage < HMP2030PSU.MIN_VOLTS:
            raise Exception(f"{voltage} is an invalid voltage (MIN={HMP2030PSU.MIN_VOLTS}).")

        if voltage > HMP2030PSU.MAX_VOLTS:
            raise Exception(f"{voltage} is an invalid voltage (MAX={HMP2030PSU.MAX_VOLTS}).")
    ...
    def setAmps(self, current):
        ...
        if amps < HMP2030PSU.MIN_AMPS:
            raise Exception(f"{amps} is an invalid current (MIN={HMP2030PSU.MIN_AMPS}).")

        if amps > HMP2030PSU.MAX_AMPS:
            raise Exception(f"{amps} is an invalid current (MAX={HMP2030PSU.MAX_AMPS}).")
...
class TENMA722550PSU(PSUBase):
    ...
    def selectOutput(self, output):
        ...
        if output != 1:
            raise Exception(f"Cannot set output {output} on {TENMA722550PSU.PSU_NAME} as it only has one output.")
        ...
        while True:
            ...
            if len(rxStr) > 0:
                try:
                    ...
                    pass
            if tries >= retry:
                raise Exception(f"{retry} failed attempt to read back the value set on the PSU.")
        ...
        while True:
            ...
            if len(rxStr) > 0:
                try:
                    ...
                    pass
            if tries >= retry:
                raise Exception("{} failed attempt to read back the value set on the PSU".format(retry))
...
class ETMXXXXP(PSUBase):
    ...
    def connect(self, port: Union[str, Tuple[str, int]]):
        ...
        self._port = port
        if len(self._port) == 2:
            self._client = ModbusTcpClient(self._port[0],
                                           self._port[1],
                                           framer=ModbusRtuFramer)
            self._debug(f"Connected to {self._port[0]}:{self._port[1]}")
        ...
        self._debug(f"Connected to {self._port}")
    ...
    def selectOutput(self, output):
        """@brief Select the output on the PSU.
           @param output The output to select."""
        if output != 1:
            raise Exception(f"Cannot set output {output} on {ETMXXXXP.PSU_NAME} as it only has one output.")

    def setVolts(self, volts):
        """@brief Set the output voltage.
           @param voltage The voltage in volts."""
        if volts < ETMXXXXP.MIN_VOLTAGE or volts > ETMXXXXP.MAX_VOLTAGE:
            raise ETMXXXXPError("{} is an invalid voltage (valid range {}V - {}V)".format(volts, ETMXXXXP.MIN_VOLTAGE, ETMXXXXP.MAX_VOLTAGE))
        self._client.write_register(ETMXXXXP.VOLTAGE_TARGET_REG_ADDR , int(volts*100.0), unit=self._unit)
    ...
    def switchOn(self, on):
        """@brief Turn on/off all previously enabled outputs.
           @param on If True turn output on."""
    ...
    def getLoadAmps(self):
        """@brief Get the selected output load current.
           @return The selected output load current in amps."""
        statsList = self.getOutputStats()
        return statsList[1]

    # Aditional methods available on this PSU that are not part of the standard
    # PSUBase interface.
