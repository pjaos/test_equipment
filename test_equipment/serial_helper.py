...
class SerialPortFinder(object):
    """@brief Responsible for providing methods useful when checking what serial ports are available on a machoine."""
    ...
    def ShowPorts(uio):
        ...
        if len(pl) > 0:
            ...
            table = [("Device Name", "Port Serial Number", "USB Port Location String")]
            for p in pl:
                table.append( (str(p.device), str(p.serial_number), str(p.location) ) )
            uio.showTable(table)
